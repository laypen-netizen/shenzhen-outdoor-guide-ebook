...
将《深圳户外景点指南》完整转换为可直接浏览的 Web 与 H5 静态产品，而不是只提供在线 PDF。331 个景点必须全部出现在网页目录中，并各有独立详情页；PDF / DOCX 仅作为附加下载格式。
...
- `places/index.html`：331 个景点的搜索与筛选目录
...
- `assets/places/`：331 张逐景点图片；`720/` 与 `960/` 子目录为生成器维护的响应式版本
...
- 校验 331 个唯一景点、10 个区域、331 个详情页、331 张图片和永久编号注册表一一对应。
...
- 331 个景点均可从目录进入独立详情页，页面内容字段完整；既有 330 个景点的网址保持不变。
- 十区专题页、关键词和筛选功能可用。
